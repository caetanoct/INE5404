...
public class KeyDemoFrame extends JFrame implements KeyListener {
	...
	public void keyReleased(KeyEvent event) {
		line4 = String.format("Tecla solta: %s", KeyEvent.getKeyText(event.getKeyCode()));
		setLines2and3(event);
	}

	@Override // trata pressionamento de uma tecla de ação
	public void keyTyped(KeyEvent event) {
		line5 = String.format("Tecla typed: %s", event.getKeyChar());
		setLines2and3(event);
	}
	...
	private void setLines2and3(KeyEvent event) {
		...
		line3 = String.format("Tecla modificador pressionada: %s", (temp.equals("") ? "none" : temp)); // modificadores
																										// de saida
		textArea.setText(String.format("%s\n%s\n%s\n%s\n%s", line1, line2, line3, line4, line5));// gera saida de três linhas de texto
	}
